...
{ // Testing key exchange
    ...
    unsigned char *PrivateKeyA, *PrivateKeyB, *PublicKeyA, *PublicKeyB, *SharedSecretA;
    ...
    //{
        ...
        send(sock, PublicKeyB, 3*2*pbytes, 0);
        valread = read( sock, PublicKeyA, 3*2*pbytes);
  ...
  CRYPTO_STATUS Status = CRYPTO_SUCCESS;
   ...
   // start server
    ...
    while (fgets(buf, sizeof(buf), command) != 0) {
      printf("%s", buf);
      if (strlen(output) + strlen(buf) > 1024){
   ...
   unsigned char key[b];
    ...
    for (int i = 0; i < b; i++){
      //      printf("%x ", SharedSecretB[i]);
      key[i] = SharedSecretB[i];
    }
    RC5_SETUP(key);
    printf("ciphertext: ");
    char testdecipher[1024];
    for (int i = 0; i < 128; i++){
      //      printf("%c ", output[i*8]);
      RC5_ENCRYPT((WORD*)output + i*8, outputWords + i*8);
      printword(outputWords[i*2]); printword(outputWords[i*2 + 1]);
      RC5_DECRYPT((WORD*)outputWords + i*8, (WORD*) testdecipher + i*8);
      //printf("%c", testdecipher[i*8]); printf("%c", testdecipher[i*8+1]); printf("%c", testdecipher[i*8+2]);
      //printf("%c", testdecipher[i*8+3]); printf("%c", testdecipher[i*8+4]); printf("%c", testdecipher[i*8+5]);
      //printf("%c", testdecipher[i*8+6]); printf("%c", testdecipher[i*8+7]);
    } printf("\n");
    send(sock , outputWords , 256, 0 );
    //    send(sock , output , strlen(output) , 0 );
   ...
   //}
    ...
    RC5_ENCRYPT(testPT, testCT);
    printf("ciphertext: ");
    for (int i = 0; i < 8; i++){
       printf("%x", testCT[i]);
    } printf("\n");
    send(sock, testCT, 8, 0);
    pclose(command); // end server ///////////////////////////////////////////////
